...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "vlm_adapter_dir": "models/vlm_adapters",
...
def api_learning_train_from_latest():
    ...
    try:
        export = _abot_learning_export_safe(
            run_id=run_id,
...
def api_learning_export():
    ...
    try:
        ...
        return jsonify(_abot_learning_export_safe(run_id=run_id, max_records=max_records, include_raw=include_raw, overwrite=str(data.get("overwrite", request.args.get("overwrite", "false"))).lower() in {"1", "true", "yes", "on"}, unique_if_exists=True))
    except Exception as exc:
...
(function(){
  ...
  async function loadStats(){
    ...
    try{
      ...
      pill.textContent = "ready - " + fmt(e.total_sft_rows) + " SFT rows - " + fmt(avail.raw_files_new_since_latest_export) + " new files" + (j.ts ? " - " + j.ts : "");
